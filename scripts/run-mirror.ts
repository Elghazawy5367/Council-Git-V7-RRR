...
function printReport(results: AnalysisResult[]): void {
  console.log(`\n${"=".repeat(80)}`);
  console.log(`${colors.bright}${colors.blue}📊 CODE MIRROR ANALYSIS REPORT${colors.reset}`);
  console.log(`${"=".repeat(80)}\n`);
  
  // Overall summary
  const totalFiles = results.length;
  const avgScore = Math.round(
    results.reduce((sum, r) => sum + r.score.overall, 0) / totalFiles
  );
  ...
  console.log(`${colors.bright}OVERALL QUALITY SCORE: ${formatScore(avgScore)}${colors.reset}`);
  console.log(`${colors.gray}Files Analyzed: ${totalFiles}${colors.reset}`);
  console.log(`${colors.gray}Total Issues: ${allGaps.length}${colors.reset}`);
  console.log(`  ${colors.red}Critical: ${criticalCount}${colors.reset}`);
  console.log(`  ${colors.yellow}High: ${highCount}${colors.reset}`);
  console.log(`  ${colors.cyan}Medium: ${allGaps.filter(g => g.severity === "medium").length}${colors.reset}`);
  console.log(`  ${colors.gray}Low: ${allGaps.filter(g => g.severity === "low").length}${colors.reset}\n`);
  
  // Category breakdown
  console.log(`${colors.bright}SCORE BREAKDOWN:${colors.reset}`);
  const avgErrorHandling = Math.round(
    results.reduce((sum, r) => sum + r.score.errorHandling, 0) / totalFiles
  );
  const avgTypeSafety = Math.round(
    results.reduce((sum, r) => sum + r.score.typeSafety, 0) / totalFiles
  );
  const avgPerformance = Math.round(
    results.reduce((sum, r) => sum + r.score.performance, 0) / totalFiles
  );
  const avgArchitecture = Math.round(
    results.reduce((sum, r) => sum + r.score.architecture, 0) / totalFiles
  );
  
  console.log(`  Error Handling: ${formatScore(avgErrorHandling)}`);
  console.log(`  Type Safety:    ${formatScore(avgTypeSafety)}`);
  console.log(`  Performance:    ${formatScore(avgPerformance)}`);
  console.log(`  Architecture:   ${formatScore(avgArchitecture)}\n`);
  
  // Top issues
  console.log(`${colors.bright}TOP ISSUES:${colors.reset}`);
  ...
  Array.from(issueFrequency.entries())
    ...
    .forEach(([issue, count], idx) => {
      console.log(`  ${idx + 1}. ${issue} ${colors.gray}(${count} occurrences)${colors.reset}`);
    });
  
  console.log();
  
  // Files needing attention (score < 60)
  const problemFiles = results
    .filter(r => r.score.overall < 60)
    .sort((a, b) => a.score.overall - b.score.overall);
  
  if (problemFiles.length > 0) {
    console.log(`${colors.bright}${colors.red}⚠️  FILES NEEDING IMMEDIATE ATTENTION:${colors.reset}`);
    problemFiles.slice(0, 10).forEach(result => {
      console.log(`\n${colors.yellow}${result.filePath}${colors.reset}`);
      console.log(`  Overall Score: ${formatScore(result.score.overall)}`);
      ...
      if (criticalGaps.length > 0) {
        ...
        criticalGaps.slice(0, 3).forEach(gap => {
          console.log(`    • ${formatSeverity(gap.severity)} - ${gap.description}`);
          console.log(`      ${colors.gray}→ ${gap.suggestion}${colors.reset}`);
        });
      }
    });
    console.log();
  ...
  const topFiles = results
    .filter(r => r.score.overall >= 80)
    .sort((a, b) => b.score.overall - a.score.overall)
    .slice(0, 5);
  
  if (topFiles.length > 0) {
    console.log(`${colors.bright}${colors.green}✨ TOP PERFORMING FILES:${colors.reset}`);
    topFiles.forEach((result, idx) => {
      console.log(`  ${idx + 1}. ${result.filePath} - ${formatScore(result.score.overall)}`);
    });
    console.log();
  }
  ...
  if (allRoleModels.size > 0) {
    console.log(`${colors.bright}📚 RECOMMENDED ROLE MODEL REPOSITORIES:${colors.reset}`);
  ...
  // Action items
  console.log(`${colors.bright}🎯 RECOMMENDED ACTIONS:${colors.reset}`);
...
async function main(): Promise<void> {
  ...
  console.log(`${colors.cyan}🔍 Scanning codebase for quality analysis...${colors.reset}\n`);
